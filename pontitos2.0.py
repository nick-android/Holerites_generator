...
# ==========================================
# MOTOR DE EXTRACCIÓN AVANZADO (TEXTO LIBRE)
# ==========================================
...
def parse_funcionarios(text):
    """Extrae datos buscando el patrón de Salario (PYG) y mapeando hacia arriba."""
    text = text.replace('|', '\n')
    ...
    for i, line in enumerate(lines):
        if "PYG" in line:
            salario_str = line.replace("PYG", "").strip()
            salario_clean = salario_str.replace('.', '').replace(',', '.')
            try:
                salario = float(salario_clean)
                ci = lines[i-1].strip()
                nombre = lines[i-2].strip()
                
                # Si el CI capturado no tiene números, ajustamos una línea más arriba
                if not re.search(r'\d', ci):
                    ci = lines[i-2].strip()
                    nombre = lines[i-3].strip()
                    
                if "FORA" not in nombre.upper() and "FORA" not in ci.upper():
                    funcionarios.append({
...
def parse_biometrico(text, df_funcionarios):
    """Agrupa el texto alrededor de cada fecha y busca coincidencias difusas de nombres."""
    roster = df_funcionarios['NOMBRE Y APELLIDO'].tolist()
    date_pattern = r'\d{2}/\d{2}/\d{4} \d{2}:\d{2}:\d{2}'
    
    # Dividir el texto aislando las fechas
    ...
    for i in range(1, len(parts), 2):
        ...
        # Crear una ventana de contexto con el texto antes y después de la marcación
        ctx_prev = parts[i-1][-120:] if i-1 >= 0 else ""
        ctx_post = parts[i+1][:120] if i+1 < len(parts) else ""
        contexto = (ctx_prev + " " + ctx_post).lower()
        ...
        # Matching de Nombres (soporta variaciones como Guilhermo/Guillermo)
        mejor_nombre = None
        max_score = 0
        
        for emp in roster:
            nombres = emp.lower().split()
            if "guillermo" in nombres: nombres.append("guilhermo")
            
            score = 0
            for n in nombres:
                if len(n) > 2 and n in contexto:
                    score += 1
            for j in range(len(nombres)-1):
                if f"{nombres[j]} {nombres[j+1]}" in contexto:
                    score += 3
...
def clasificar_minutos(marcaciones_dia):
    ...
    faltas_mins, e50_mins, e100_mins = 0, 0, 0
    ...
    return faltas_mins, e50_mins, e100_mins

def calcular_nomina(df_func, df_bio):
    resultados = []
    
    # Extraer los cargos detectados en el biométrico
    ...
    for _, empleado in df_func.iterrows():
        ...
        fechas_unicas = marcaciones_empleado['Tiempo'].dt.date.unique()
        ...
        for fecha in fechas_unicas:
            ...
            f, e50, e100 = clasificar_minutos(marcs_dia)
            total_faltas += f
            total_e50 += e50
            total_e100 += e100
        ...
        imponible = salario_base - desc_faltas + monto_e50 + monto_e100
...
class PDFHolerite(FPDF):
    ...
    def draw_receipt_half(self, data, is_empresa=False, offset_y=0):
        ...
        self.set_font('helvetica', '', 10)
        self.cell(100, 5, data['Funcionario'].upper(), 0, 1)
        ...
        self.set_font('helvetica', '', 10)
        self.cell(70, 5, data['Cargo'].upper(), 0, 1)
        ...
        if data['Descuento Faltas'] > 0:
            add_row('5', 'Faltas y Atrasos', f"{data['Faltas/Tardanzas (Mins)']} mins", 0, data['Descuento Faltas'])
        ...
        self.set_font('helvetica', '', 8)
        self.cell(100, 4, data['Funcionario'].upper(), 0, 1)
...
if pdf_func and pdf_bio:
    if st.button("Procesar Archivos y Calcular", type="primary"):
        with st.spinner("Leyendo estructura libre y calculando..."):
            try:
                # Extraer como texto plano para evitar el error de estructura de tabla
                text_func = extract_text_from_pdf(pdf_func)
                text_bio = extract_text_from_pdf(pdf_bio)
                
                df_funcionarios = parse_funcionarios(text_func)
...
if 'resultados' in st.session_state:
    ...
    st.dataframe(
        st.session_state['resultados'][['Funcionario', 'Cargo', 'Descuento Faltas', 'Monto Extra 50%', 'Monto Extra 100%', 'IPS (9%)', 'Salario Líquido Final']],
